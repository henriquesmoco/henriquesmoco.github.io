...
(function($){
    $.fn.toc = function(options) {
      ...
      if (!headers.length || headers.length < settings.minimumHeaders || !output.length) {
        $(this).hide();
        $('.post-toc-title').css('display', 'none');
        return;
      }
      ...
      this_level,
      html = "<p class=\"toc-title-section\"><strong class=\"toc-title\">" + settings.title + "</strong></p>\n";
  ...
  $(document).ready(function(){
    ...
    //var fixmeTop = $('#post-toc-module').offset().top;
    ...
    //       width: 'inherit'
    //     });
    //     $('.post-toc').css('overflow', 'auto');
    //   } else {
    //     $('#post-toc-module').css({
    //       position: 'inherit',
    //       width: 'inherit'
    //     });
    //     $('.post-toc').css('overflow', 'hidden');
    //     $('.post-toc').scrollTop(0);
    //   }
    ...
    //Hide the TOC title because we are using a dropdown
